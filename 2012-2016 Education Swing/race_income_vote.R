...
res.16 <- glmer(vote ~ (1|race) + (1|income) + (1|state:race) + (1|race:income) + (1|state:income) +  degree + black + hispanic + dem16,df16,family=binomial(link='logit'))
summary(res.16)

res.12 <- glmer(vote ~ (1|race) + (1|income) + (1|state:race) + (1|race:income) + (1|state:income) + (1|state:income:race) + degree + black + hispanic + dem12,df12,family=binomial(link='logit'))
summary(res.12)

output.16 <- expand_grid(state=unique(df16$state),income=levels(df16$income),race=levels(df16$race)) %>%
  inner_join(context16)
output.16$clinton <- predict(res.16,output.16,allow.new.levels=T,type='response')

output.12 <- expand_grid(state=unique(df12$state),income=levels(df12$income),race=levels(df12$race)) %>%
  inner_join(context12)
output.12$obama <- predict(res.12,output.12,allow.new.levels=T,type='response')

output <- expand_grid(state=unique(df16$state),income=levels(df16$income),race=levels(df16$race)) %>%
  inner_join(select(output.12,state,income,race,obama)) %>%
  inner_join(select(output.16,state,income,race,clinton)) %>%
...
data %>%
  ...
  filter(!is.na(income),!is.na(race)) %>%
  ...
  labs(title='Swing between 2012 and 2016 presidential elections by race and income',
       fill='Swing') +
  theme(plot.title = element_text(hjust = 0.5)) +
  ggsave('race_income_swing.png',width=16*1.5,height=7*1.5,type='cairo')
